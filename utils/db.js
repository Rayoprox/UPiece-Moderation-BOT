...
const db = {
    ...
    ensureTables: async () => {
        ...
        await db.query(`CREATE TABLE IF NOT EXISTS ticket_panels (id SERIAL PRIMARY KEY, guild_id TEXT NOT NULL, panel_id TEXT NOT NULL, title TEXT DEFAULT 'Support Ticket', description TEXT DEFAULT 'Click below to open a ticket.', banner_url TEXT, button_label TEXT DEFAULT 'Open Ticket', button_style TEXT DEFAULT 'Primary', button_emoji TEXT DEFAULT '📩', panel_color TEXT DEFAULT '#5865F2', welcome_color TEXT DEFAULT '#5865F2', support_role_id TEXT, blacklist_role_id TEXT, ticket_category_id TEXT, log_channel_id TEXT, welcome_message TEXT DEFAULT 'Hello {user}, staff will be with you shortly.', ticket_limit INTEGER DEFAULT 1, created_at BIGINT DEFAULT EXTRACT(EPOCH FROM NOW()) * 1000, UNIQUE (guild_id, panel_id));`);        
