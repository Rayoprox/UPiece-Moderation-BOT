...
async function verifyAppealEligibility(userId, mainGuild, db) {
    ...
    if (banLog.rows[0]?.endsat) {
        ...
        return { valid: false, message: `Temporary bans are not appealable. Expires: <t:${endsAtTimestamp}:f>.` };
    }
...
const generateLogEmbed = (logs, targetUser, page, totalPages, authorId, isWarningLog = false, logsPerPage = 5) => {
    ...
    const embed = new EmbedBuilder()
        ...
        .setTitle(`${isWarningLog ? emojis.warn || '⚠️' : emojis.info || 'ℹ️'} ${isWarningLog ? 'Warnings' : 'Moderation Logs'} for ${targetUser.tag}`)
